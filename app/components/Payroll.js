...
import { insertIncome, getIncomes } from './CRUD';
import {
  View,
  Text,
  StyleSheet,
...
const Payroll = () => {
  ...
  const handleAddExpense = () => {
    if (newExpense.xarajat && newExpense.price) {
      ...
      const kirim = newExpense.xarajat;
  ...
  const totalAmount = incomes.reduce((sum, item) => sum + item.price, 0);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.sectionTitle}>Kirim</Text>
  ...
  <Text style={styles.sectionTitle}>Barcha Kirimlar</Text>
  {incomes.map((item) => (
    <View key={item.key} style={styles.expenseItem}>
      <View style={styles.left}>
        <Text style={styles.name}>{item.kirim}</Text>
        <Text style={styles.date}>{item.date}</Text>
      </View>
      <View style={styles.right}>
        <Text style={styles.status}>Kirim</Text>
        <Text style={styles.total}>${item.price}</Text>
      </View>
    </View>
  ))}
      ...
      <Modal visible={openModal} transparent animationType="slide">
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>Kirim qo'shish</Text>

            <TextInput
              style={styles.input}
              placeholder="Kirim haqida yozing"
